...
def bloque_ejecucion(units, epochs, batch_size, adam_opt, cn, pi, path_full_dataset_processed, path_scores_dataset_processed, path_models_saved, execution_number, pacientes, posicion_glucosa):
    ...
    for paciente in pacientes:
        ...
        for exe in range(execution_number):    #duda, execution antes de bloque matriz, separar x y y dividir datos o depues.
            ...
            y_pred_exp_1, score_exp_1 = ejecutaModeloSinPrint(units, epochs, batch_size, adam_opt, path_models_saved, xTrain_DeltaInsulin, yTrain, xVal_DeltaInsulin, yVal, xTest_DeltaInsulin, yTest)
            # y_pred_exp_2, score_exp_2 = ejecutaModeloSinPrint(units, epochs, batch_size, adam_opt, path_models_saved, xTrain_Insulin, yTrain, xVal_Insulin, yVal, xTest_Insulin, yTest)
            # y_pred_exp_3, score_exp_3 = ejecutaModeloSinPrint(units, epochs, batch_size, adam_opt, path_models_saved, xTrain_Insulin_lispro, yTrain, xVal_Insulin_lispro, yVal, xTest_Insulin_lispro, yTest)
            # _pred_exp_4, score_exp_4 = ejecutaModeloSinPrint(units, epochs, batch_size, adam_opt, path_models_saved, xTrain_Insulin_comidasDeltas, yTrain, xVal_Insulin_comidasDeltas, yVal, xTest_Insulin_comidasDeltas, yTest)
            # y_pred_exp_5, score_exp_5 = ejecutaModeloSinPrint(units, epochs, batch_size, adam_opt, path_models_saved, xTrain_Insulin_comidasExp, yTrain, xVal_Insulin_comidasExp, yVal, xTest_Insulin_comidasExp, yTest)
            # y_pred_exp_6, score_exp_6 = ejecutaModeloSinPrint(units, epochs, batch_size, adam_opt, path_models_saved, xTrain_Insulin_comidasExp_lispro, yTrain, xVal_Insulin_comidasExp_lispro, yVal, xTest_Insulin_comidasExp_lispro, yTest)
            # y_pred_exp_7, score_exp_7 = ejecutaModeloSinPrint(units, epochs, batch_size, adam_opt, path_models_saved, xTrain_Insulin_comidasExp_profiles, yTrain, xVal_Insulin_comidasExp_profiles, yVal, xTest_Insulin_comidasExp_profiles, yTest)
        ...
        df = pd.DataFrame(np.array(listaScores),
                          columns=['score_exp_1', 'score_exp_2', 'score_exp_3', 'score_exp_4', 'score_exp_5', 'score_exp_6', 'score_exp_7'], index=execution_list)
        print(df)
